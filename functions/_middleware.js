...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function raceLoadImage(img, wrap) {
        ...
        // 构建 R2 目标链接：https://r2.setutime.com/[分类前缀]/pic-[期号]-[序号].webp
        let r2Src = null;
    ...
    if ('IntersectionObserver' in window) {
        ...
        }, { rootMargin: "0px 0px 5000px 0px" }); // 5000px 快速滚动缓冲区
    ...
    // --- 保留你原本 HTML 里处理“下一期链接”、“保存按钮”和“底部悬浮按钮”的逻辑 ---
    const titleEl = document.querySelector('.title');
    if (titleEl) {
        const match = titleEl.innerText.match(/\\d+/);
        if (match) {
...
`;
  ...
  // 3. 使用 HTMLRewriter：
  //    - 移除 HTML 原本自带的那个 <script>（防止冲突和双重加载）
  //    - 在 </body> 结束前注入合二为一的新 <script>
  return new HTMLRewriter()
    .on("script", {
      element(element) {
        // 如果发现这个 script 标签包含原本的懒加载逻辑，直接把它从 HTML 中删掉
        element.remove();
      }
    })
    .on("body", {
